/* eslint-disable */
import React, { Fragment, isValidElement, cloneElement, createRef, Component } from 'react';
...
var getAutoControlledStateValue = function getAutoControlledStateValue(propName, props, state, includeDefaults) {
    ...
    // regular props
    var propValue = props[propName];
    if (!isUndefined_1(propValue))
        return propValue;
    if (includeDefaults) {
        // defaultProps
        var defaultProp = props[getDefaultPropName(propName)];
        if (!isUndefined_1(defaultProp))
            return defaultProp;
        // initial state - state may be null or undefined
        if (state) {
            var initialState = state[propName];
            if (!isUndefined_1(initialState))
                return initialState;
        }
    }
 ...
 * @template Props
 */
var AutoControlledManager = /** @class */ (function () {
    /**
     ...
     */
    function AutoControlledManager(autoControlledProps, propsToStateDerivers) {
        if (propsToStateDerivers === void 0) { propsToStateDerivers = {}; }
        var getInitialAutoControlledState = propsToStateDerivers.getInitialAutoControlledState, getAutoControlledStateFromProps = propsToStateDerivers.getAutoControlledStateFromProps;
        ...
        this.getDerivedStateFromProps = function getDerivedStateFromProps(nextProps, prevState) {
            ...
            var newStateFromProps = autoControlledProps.reduce(function (acc, prop) {
                ...
                if (!isUndefined_1(nextProps[prop])) {
            ...
            if (isFunction_1(getAutoControlledStateFromProps)) {
                var computedState = getAutoControlledStateFromProps(nextProps, __assign$1({}, prevState, newStateFromProps));
                // We should follow the idea of getDerivedStateFromProps() and return only modified state
                return __assign$1({}, newStateFromProps, computedState);
            }
            return newStateFromProps;
        };
        this.getInitialAutoControlledStateFromProps = function getInitialAutoControlledStateFromProps(props) {
            var state = isFunction_1(getInitialAutoControlledState) ? getInitialAutoControlledState(props) : {};
            var initialAutoControlledState = autoControlledProps.reduce(function (acc, prop) {
                acc[prop] = getAutoControlledStateValue(prop, props, state, true);
                return acc;
            }, {});
            var initialState = __assign$1({}, state, initialAutoControlledState);
            return initialState;
        };
    ...
    AutoControlledManager.prototype.trySetState = function (maybeState, callback) {
        var _this = this;
        var newState = Object.keys(maybeState).reduce(function (acc, prop) {
            // ignore props defined by the parent
            if (!isUndefined_1(_this.props[prop])) {
                return acc;
            }
...
var transitionModalAutoControlledManager = new AutoControlledManager([
    'open',
], {
    getInitialAutoControlledState: function () {
        return {
            open: false,
        };
    },
});
var TransitionModal = /** @class */ (function (_super) {
    __extends(TransitionModal, _super);
    function TransitionModal() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.modal = createRef();
        _this.state = transitionModalAutoControlledManager.getInitialAutoControlledStateFromProps(_this.props);
        _this.trySetState = transitionModalAutoControlledManager.trySetState;
        _this.applyTransformOriginToModal = function (point) { return function () {
            ...
            var _a = _this.props, animation = _a.animation, zoomFromMousePositionOnOpen = _a.zoomFromMousePositionOnOpen;
            var isZoom = animation === 'zoom';
            var modal = _this.modal.current;
            if (!modal || !(zoomFromMousePositionOnOpen && isZoom)) {
                return;
            }
            var dialog = modal.ref.current;
            var dialogOffset = offset(dialog);
            var transformOrigin = {
                x: point.x - dialogOffset.left,
                y: point.y - dialogOffset.top,
            };
            var transformOriginStr = toPxString(transformOrigin.x) + " " + toPxString(transformOrigin.y);
            setTransformOrigin(dialog, transformOriginStr);
        }; };
        _this.handleOpen = function (event) {
            safeInvoke(_this.props.onOpen, event, _this.props);
            _this.trySetState({
                open: true,
            }, _this.applyTransformOriginToModal({
                x: event.clientX,
                y: event.clientY,
            }));
        };
        _this.handleClose = function (event) {
            safeInvoke(_this.props.onClose, event, _this.props);
            _this.trySetState({
                open: false,
            });
        };
    ...
    TransitionModal.prototype.render = function () {
        var open = this.state.open;
        var _a = this.props, 
        // Ignored / Overridden props
        trigger = _a.trigger, // Render this outside of the modal.
        onOpen = _a.onOpen, // Use class implementation.
        onClose = _a.onClose, // Use class implementation.
        unmountOnHide = _a.unmountOnHide, // Always `true`.
        _open = _a.open, // Use class state.
        _visible = _a.visible, // Use class state.
        zoomFromMousePositionOnOpen = _a.zoomFromMousePositionOnOpen, // For styling.
        // #region Transition component props
        animation = _a.animation, directional = _a.directional, duration = _a.duration, mountOnShow = _a.mountOnShow, onComplete = _a.onComplete, onHide = _a.onHide, onShow = _a.onShow, onStart = _a.onStart, reactKey = _a.reactKey, transitionOnMount = _a.transitionOnMount, 
        // #endregion
        modalProps = __rest(_a, ["trigger", "onOpen", "onClose", "unmountOnHide", "open", "visible", "zoomFromMousePositionOnOpen", "animation", "directional", "duration", "mountOnShow", "onComplete", "onHide", "onShow", "onStart", "reactKey", "transitionOnMount"]);
        var transitionProps = {
            animation: animation,
        ...
        return (React.createElement(Fragment, null,
            ...
            , __assign({}, transitionProps, { 
                // Control these props.
                unmountOnHide: true, visible: open }),
                React.createElement(Modal, __assign({}, modalProps, { ref: this.modal, onClose: this.handleClose, open: true })))));
    };
    // Infer the default props from the subcomponents.
    TransitionModal.defaultProps = __assign(__assign({}, Transition.defaultProps), Modal.defaultProps);
    // Infer the prop types from the subcomponents.
    TransitionModal.propTypes = __assign(__assign(__assign({}, Transition.propTypes), Modal.propTypes), { zoomFromMousePositionOnOpen: propTypes.bool });
    // Export these components from the original component so that there's no need to import <Modal>.
    TransitionModal.Header = Modal.Header;
    TransitionModal.Content = Modal.Content;
    TransitionModal.Description = Modal.Description;
    TransitionModal.Actions = Modal.Actions;
    TransitionModal.getDerivedStateFromProps = transitionModalAutoControlledManager.getDerivedStateFromProps;
